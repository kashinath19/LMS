 ...
 *  - footerUser: { name, email, initials, role, avatarUrl }
 *  - onLogout: fn
 *  - open: bool (optional) - controls mobile drawer open state
 *  - onClose: fn (optional) - called when the overlay or close button clicked
 */
const Sidebar = ({ logo, navItems = [], footerUser: footerUserProp, onLogout, open = false, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        ...
        const displayName = username || (email ? (email.split('@')[0] || email) : null);
        ...
        const fallbackEmail = localStorage.getItem('user_email');
        const fallbackName = localStorage.getItem('user_name') || (fallbackEmail ? fallbackEmail.split('@')[0] : null);
        if (mounted) setAdminProfile({ id: userId, name: fallbackName, username: null, email: fallbackEmail });
      }
    };
  ...
  const footerUser = (() => {
    ...
    if (role === 'admin') {
      ...
      const name = adminProfile?.name || nameFromStorage || (emailFromStorage ? emailFromStorage.split('@')[0] : null);
      const email = adminProfile?.email || emailFromStorage || null;
      ...
      return {
        ...
        initials: name ? name.split(' ').map(n => n[0]).slice(0, 2).join('') : null
      };
    }
    return footerUserProp;
  })();
            ...
            <div className={styles.userBlock}>
              <div className={styles.avatar}>
                {footerUser.avatarUrl ? (
                  ...
                  <span>{(footerUser.initials || (footerUser.name || footerUser.email || 'U').slice(0, 2)).toUpperCase()}</span>
                )}
              </div>
